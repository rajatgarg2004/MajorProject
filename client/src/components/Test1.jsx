...
const Test = () => {
  ...
  // Initialize the timetable once during component mount
const [timetable, setTimetable] = useState(() =>
  Array(5).fill().map(() => ({
    slots: Array(9).fill([]), // Initialize each slot as an empty array
  }))
);
  ...
  const handleSlotClick = (dayIndex, periodIndex) => {
    setSelectedSlot({ dayIndex, periodIndex, subject: '' });
  };
  ...
  const handleSubjectAssign = () => {
    ...
    // Check if the room is already assigned in the same period across all days
    const isRoomOccupied = updatedTimetable.some((day) =>
      day.slots[periodIndex] && // Check if there is any slot in the same period
      day.slots[periodIndex].some((entry) => entry.room === selectedRoom) // Check if the room is already occupied
    );
  
    if (isRoomOccupied) {
      setErrorMessage('This room is already occupied during this period.');
      return;
    }
  
    // Fetch the current slot for the selected day and period
    const currentSlot = updatedTimetable[dayIndex].slots[periodIndex];
  
    // Check if the subject is already assigned to this slot (same day, same period)
    const isDuplicate = currentSlot && currentSlot.some(
      (entry) =>
    ...
    if (isDuplicate) {
      setErrorMessage('This subject is already assigned to this slot.');
      return;
    }
  
    // Add the new subject to the slot
    updatedTimetable[dayIndex].slots[periodIndex] = [
      ...currentSlot,
      {
        subject: subjects.find((sub) => sub.name === selectedSlot.subject).name,
        type: subjectType,
        room: selectedRoom,
      },
    ];
            ...
            {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].map((day, dayIndex) => (
              <tr key={dayIndex} >
                ...
                {timetable[dayIndex]?.slots.map((slot, periodIndex) => (
                  <td
                    key={periodIndex}
                    className="p-3 border border-gray-600 hover:bg-gray-600"
                    onClick={() => handleSlotClick(dayIndex, periodIndex)} // Pass both indices
                  >
                    {/* Render multiple courses in the slot */}
                    {slot.length > 0
                      ? slot.map((entry, i) => (
                        <div key={i} className='border p-2 cursor-pointer border-white'>
                          {entry.subject} - {entry.type} ({entry.room})
                        </div>
                      ))
                      : 'Empty'}
                  </td>
